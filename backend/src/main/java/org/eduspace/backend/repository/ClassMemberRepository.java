...
public interface ClassMemberRepository extends JpaRepository<ClassMember, Long> {
    Optional<ClassMember> findFirstByUserIdAndCourseClassId(Long userId, Long classId);

    List<ClassMember> findByUserId(Long userId);

    boolean existsByUserIdAndContextRole(Long userId, String contextRole);
    ...
    List<ClassMember> findByUserIdAndContextRoleIn(Long userId, List<String> contextRoles);

    Optional<ClassMember> findByUserIdAndCourseClassIdAndContextRole(Long userId, Long classId, String contextRole);

    @Query("SELECT cm FROM ClassMember cm WHERE cm.user.id = :userId AND cm.courseClass.id = :classId AND cm.contextRole IN :roles")
    Optional<ClassMember> findByUserIdAndCourseClassIdAndContextRoleIn(
            @Param("userId") Long userId,
    ...
    @Query("""
                ...
                WHERE cm.courseClass.id = :classId
                  ...
                  AND cm.contextRole IN ('MENTOR', 'CREATOR')
            """)
    Optional<ClassMember> findMentorOrCreatorInClass(
            @Param("classId") Long classId,
            @Param("userId") Long userId);
    ...
    @Query("""
                ...
                WHERE cm.courseClass.id = :classId
                  AND cm.contextRole IN ('MENTOR', 'CREATOR')
            """)
    List<ClassMember> findAllMentorsInClass(@Param("classId") Long classId);
}
