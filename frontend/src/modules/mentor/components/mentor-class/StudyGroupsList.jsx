...
const StudyGroupsList = ({ pairs }) => {
  const getGroupDiagnostic = (pair, memberProgresses) => {
    const minProgress =
      memberProgresses.length > 0 ? Math.min(...memberProgresses) : 0
    const maxProgress =
      memberProgresses.length > 0 ? Math.max(...memberProgresses) : 0
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {pairs.map((pair) => {
          const memberProgresses = pair.members
            ? pair.members.map((m) => m.progress ?? 0)
            : []
          const minProgress =
            memberProgresses.length > 0 ? Math.min(...memberProgresses) : 0
            ...
            >
              <CardContent className="p-5 flex flex-col md:flex-row items-stretch justify-between gap-6">
                ...
                <div className="flex flex-col justify-between shrink-0 md:w-36">
                  <div>
                    <p className="font-extrabold text-neutral-dark text-base">
                      Nhóm #{pair.studyGroupId}
                    </p>
                    <p className="text-[10px] text-neutral-medium font-bold mt-0.5">
                      ID: PAIR-0{pair.studyGroupId}
                    </p>
                  </div>
                  <div className="mt-3">
                    <Badge
                    ...
                    pair.members.map((member, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          <span
                            className="text-xs font-bold text-neutral-dark truncate w-full mt-1.5"
                ...
                <div className="flex flex-col justify-between items-end shrink-0 md:w-44 text-right">
                  <div>
                    <span className="text-[10px] font-bold text-neutral-medium uppercase tracking-wider block">
                      Chẩn đoán
                    </span>
                    <span className="text-xs font-bold block mt-1 text-emerald-700">
                      {diagnostic}
                    </span>
                  </div>
