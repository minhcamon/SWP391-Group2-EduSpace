...
export const MentorClassCard = ({ classItem }) => {
  const { id, name, courseTitle, status, numberOfPairs } = classItem
    ...
    <section className="bg-white rounded-2xl p-6 shadow-sm border border-border-light/35 flex flex-col transition-all hover:shadow-md duration-200">
      ...
      <div className="flex justify-between items-start mb-5">
        <div className="flex flex-col gap-1.5 min-w-0">
          <h2 className="text-lg font-bold text-neutral-dark truncate">
            {name}
          </h2>
          <div className="flex flex-wrap gap-2 items-center">
            <Badge
              variant="roletag"
              className="text-[10px] px-2.5 py-0.5 uppercase shrink-0"
            >
              Mentor
            </Badge>
            <span
              className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full shrink-0 ${statusInfo.color}`}
            >
              {statusInfo.label}
            </span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6 grow">
        <div className="bg-bg-base border border-border-light/20 p-3.5 rounded-xl text-center">
          <p className="text-[11px] font-semibold text-neutral-medium mb-1.5">
            Số cặp học viên
          </p>
      ...
      <div className="pt-4 border-t border-border-light/25">
        <Link
          to={`/mentor/classes/${id}`}
          className="block w-full"
        >
          <Button
            variant="secondary"
            className="w-full h-10 text-xs font-bold py-2 hover:scale-[0.98] transition-all"
          >
            Vào quản lý lớp học
          </Button>
        </Link>
