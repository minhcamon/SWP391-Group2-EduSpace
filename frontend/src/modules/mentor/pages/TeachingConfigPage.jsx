      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-medium mt-1">
            Thay đổi trạng thái sẵn sàng nhận lớp cho các khóa học bạn đã được chứng nhận.
          </p>
        </div>
        <button
          onClick={refresh}
      ...
      {activeCourses.length === 0 ? (
        <div className="bg-white border border-border-light/40 rounded-3xl p-12 text-center shadow-sm">
          ...
          <p className="text-sm text-neutral-medium max-w-md mx-auto leading-relaxed">
            Bạn cần hoàn thành các khóa học và nhận chứng chỉ (Certificates) để
            có thể tham gia giảng dạy. Hãy tiếp tục học tập và đạt chứng chỉ nhé!
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeCourses.map((course) => {
            ...
            return (
              <Card
                key={course.courseId}
                className="group overflow-hidden rounded-2xl border transition-all duration-300 shadow-sm hover:shadow-md bg-white border-border-light/40"
              >
                <CardContent className="p-6 flex flex-col justify-between h-full min-h-[200px]">
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-3.5">
                      <Badge
                        variant="outline"
                        className="bg-emerald-600/5 text-emerald-600 border-emerald-600/10 text-[10px] font-bold px-2 py-0.5 uppercase tracking-wide"
                      >
                  ...
                  <div className="mt-6 pt-4 border-t border-border-light/35 flex items-center justify-between gap-2">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-1.5 mt-1.5">
                        ...
                        />
                        <span
                          className={`text-xs font-bold ${isAvailable ? 'text-emerald-700' : 'text-amber-700'}`}
                        >
                          {isAvailable
                            ? 'Sẵn sàng nhận lớp'
                            : 'Đang bận / Tạm nghỉ'}
