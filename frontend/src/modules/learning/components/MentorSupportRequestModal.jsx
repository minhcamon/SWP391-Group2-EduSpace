...
const MentorSupportRequestModal = ({ isOpen, onClose, courseId, classId, studyGroupId, studyGroup }) => {
            ...
            try {
                ...
                if (dashboard && dashboard.modules) {
                    dashboard.modules.forEach(m => {
                        if (m.assignment && m.assignment.submissionId) {
                            list.push({
                                submissionId: m.assignment.submissionId,
                ...
                console.error("Lỗi tải danh sách bài tập đã nộp:", error)
            } finally {
    ...
    const handleSubmissionSelect = async (submissionId) => {
        if (!submissionId) {
            handleChange('reportedUserId', null)
            setGraderName('')
            return
        }
        ...
        try {
            const review = await learnService.getSubmissionReview(classId, selectedSub.assignmentId)
            if (review && review.reviewerUserId) {
                handleChange('reportedUserId', review.reviewerUserId)
                setGraderName(review.reviewerName || 'Thành viên chấm chéo')
            } else {
                handleChange('reportedUserId', null)
                setGraderName('')
            }
        } catch (error) {
            console.error("Lỗi lấy thông tin người chấm bài:", error)
            handleChange('reportedUserId', null)
            setGraderName('')
        }
    }
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedIncidentType?.requiresSubmission && !formData.submissionId) {
            toast.error('Vui lòng chọn bài tập cần khiếu nại')
            return
        }
  ...
  "
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
                    ...
                    {/* Submission Selection for ASSIGNMENT_DISPUTE */}
                    {selectedIncidentType?.requiresSubmission && (
                        <div>
                            <label className="block text-sm font-semibold text-neutral-dark mb-2">
                                Bài tập cần khiếu nại <span className="text-error">*</span>
                            </label>
                            {isLoadingSubmissions ? (
                                <div className="text-sm text-neutral-medium flex items-center gap-2 py-2">
                                    <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
                                    Đang tải danh sách bài tập...
                                </div>
                            ) : submissions.length === 0 ? (
                                <div className="p-3 bg-red-50 border border-red-200 rounded-xl">
                                    <p className="text-sm text-error font-semibold">Bạn chưa có bài nộp nào trong lớp học này để khiếu nại.</p>
                                </div>
                            ) : (
                                <>
                                    <select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            handleChange('submissionId', val);
                                            handleSubmissionSelect(val);
                                        }}
                                        className="w-full px-4 py-3 border border-border-light rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all bg-white"
                                        required
                                    >
                                        <option value="">-- Chọn bài tập cần khiếu nại --</option>
                                        {submissions.map((sub) => (
                                            <option key={sub.submissionId} value={sub.submissionId}>
                                                {sub.title} ({sub.status === 'GRADED' ? 'Đã chấm' : 'Chưa chấm / Chờ chấm'})
                                            </option>
                                        ))}
                                    </select>
                                    {graderName && (
                                        <div className="bg-primary/5 border border-primary/20 p-3.5 rounded-xl mt-2.5 flex items-center justify-between">
                                            <span className="text-xs text-neutral-medium font-semibold">Người chấm bài (Người bị khiếu nại):</span>
                                            <span className="text-sm font-bold text-primary">{graderName}</span>
                                        </div>
                                    )}
                                </>
                            )}
