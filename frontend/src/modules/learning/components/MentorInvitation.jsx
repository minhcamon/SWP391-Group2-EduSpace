...
export const MentorInvitation = ({ isOpen, onClose, classId }) => {
  ...
  const handleBecomeMentor = async () => {
    ...
    try {
      ...
      toast.success('Đăng ký ứng tuyển Mentor thành công!', {
        description:
          'Chúng tôi sẽ xem xét thành tích học tập và liên hệ sớm nhất.'
      })
      onClose()
    } catch (error) {
  ...
  return (
    ...
    >
      <DialogContent
        className="w-full min-w-2xl bg-white dark:bg-card rounded-2xl overflow-hidden shadow-2xl p-0 gap-0 border border-slate-100 text-left"
      ...
      >
        ...
        <header className="bg-neutral-dark p-8 flex flex-col items-center text-center">
          ...
          <DialogTitle className="text-lg md:text-xl font-extrabold text-white leading-tight">
            Trở thành Mentor thế hệ tiếp theo, tại sao không?
          </DialogTitle>
          <DialogDescription className="sr-only">
            Lời mời ứng tuyển làm Mentor lớp học dựa trên kết quả học tập xuất
            sắc của bạn.
          </DialogDescription>
        </header>
        ...
        <div className="p-8 space-y-6">
          <p className="text-xs md:text-sm leading-relaxed text-neutral-medium text-center font-medium">
            Chúc mừng{' '}
            <strong className="text-primary font-bold">{userName}</strong>, với
            thành tích học tập xuất sắc và sự tương tác tích cực trong khóa học
            vừa qua, bạn hoàn toàn đủ điều kiện để trở thành Mentor dẫn dắt các
            bạn học viên khóa sau.
          </p>
