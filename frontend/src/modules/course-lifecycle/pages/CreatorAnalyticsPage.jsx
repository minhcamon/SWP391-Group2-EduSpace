...
export default function CreatorAnalytics() {
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-125 px-4">
        <div className="text-center p-8 bg-white rounded-2xl border border-border-light/40 shadow-sm max-w-md">
          ...
          <p className="text-sm text-neutral-medium mb-6">{error}</p>
        </div>
      </div>
    )
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
          <CardHeader className="p-0 flex-1">
            ...
            <CardDescription className="text-sm text-neutral-medium mt-1">
              Đánh giá toàn diện lượng người học, tỷ lệ hoàn thành chương trình,
              và phân bổ điểm số.
            </CardDescription>
          </CardHeader>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-neutral-dark">
                  Xu hướng tham gia khóa học
                </h3>
                <p className="text-[11px] text-neutral-medium">
                  Lượng người tham gia mới đăng ký theo tháng
                </p>
              </div>
              ...
              >
                ...
                {monthlyTrends.map((trend, idx) => {
                  ...
                  const height = Math.max(
                    10,
                    Math.round((trend.count / maxVal) * 180)
                  )
                  const y = 200 - height
                  ...
                  return (
                    <g key={trend.month}>
                      <rect
                        ...
                        height={height}
                        rx="4"
                        className={`fill-primary/${20 + idx * 15} hover:fill-primary transition-all duration-300 cursor-pointer ${isLast ? 'animate-pulse' : ''}`}
                      />
                      <text
                        x={x + 16}
                        y={y - 8}
                        className="text-[9px] fill-primary font-bold opacity-0 hover:opacity-100 transition-opacity"
                        textAnchor="middle"
                      >
                        {trend.count}
                      </text>
                      <text
                        x={x + 16}
                        y={215}
                        className="text-[10px] fill-neutral-medium font-bold"
                        textAnchor="middle"
                      >
                        {trend.month}
                      </text>
            ...
            <div className="flex justify-center items-center relative py-2">
              ...
              >
                ...
                <circle
                  ...
                  stroke="#10b981"
                  strokeWidth="4.5"
                  strokeDasharray={`${stats.passRate || 0} ${100 - (stats.passRate || 0)}`}
                  strokeDashoffset="0"
                ...
                <circle
                  ...
                  stroke="#ef4444"
                  strokeWidth="4.5"
                  strokeDasharray={`${stats.failRate || 0} ${100 - (stats.failRate || 0)}`}
                  strokeDashoffset={`-${stats.passRate || 0}`}
                  className="transition-all duration-1000"
                ></circle>
                {/* Dropped segment */}
                <circle
                  ...
                  stroke="#f97316"
                  strokeWidth="4.5"
                  strokeDasharray={`${stats.dropRate || 0} ${100 - (stats.dropRate || 0)}`}
                  strokeDashoffset={`-${(stats.passRate || 0) + (stats.failRate || 0)}`}
                  className="transition-all duration-1000"
                ></circle>
              </svg>
              {/* Inside Center Text */}
              <div className="absolute flex flex-col items-center justify-center">
                <span className="text-xl font-extrabold text-neutral-dark">
                  {stats.passRate}%
                </span>
            ...
            <div className="space-y-2 text-xs font-semibold">
              <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                ...
                <span className="text-neutral-dark font-bold">
                  {stats.passedCount} ({stats.passRate}%)
                </span>
              </div>
              <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                <div className="flex items-center gap-2">
                ...
                <span className="text-neutral-dark font-bold">
                  {stats.failedCount} ({stats.failRate}%)
                </span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                ...
                <span className="text-neutral-dark font-bold">
                  {stats.droppedCount} ({stats.dropRate}%)
                </span>
              </div>
            </div>
          </CardContent>
